...
priorduration = 5
# stairs = data.QuestHandler(priorduration, 0.1, pThreshold=0.82, gamma=0.01, nTrials=100, minVal=5, maxVal=1000)
...
Wantlist_Contrast=True
# do it !!!
#  =========== main experiment loop ========
if Wantlist_Contrast == True:
 for condition_contrast in Contrastlist:
   stairs = data.QuestHandler(priorduration, 0.1, pThreshold=0.82, gamma=0.01, nTrials=100, minVal=5, maxVal=1000)
   for thisDur in stairs: # loop trial handler
    ...
    driftGrating.size = 8 # set size as diameter
    ...
    for iFrame in range(int(thisDur)): # loop frames
        ...
        driftGrating.contrast = condition_contrast
    ...
    for condition_contrast in Contrastlinear:
        stairs = data.QuestHandler(priorduration, 0.1, pThreshold=0.82, gamma=0.01, nTrials=100, minVal=5, maxVal=1000)
        for thisDur in stairs:  # loop trial handler
            # show fixation
            fixation.draw()
